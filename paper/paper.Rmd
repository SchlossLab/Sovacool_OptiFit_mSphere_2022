...
- **start paragraph with question, end with why we should care. transitions to move the story along.**
...
For comparison to the state of the art, clustering datasets _de novo_ with
OptiClust produced an average MCC score of `r opticlust_mcc`.
This gap in OTU quality mostly disappears when clustering in open reference
...
Thus, open reference OptiFit produces OTUs of very similar quality as _de novo_
clustering, and closed reference OptiFit follows closely behind as long as a
suitable reference database is chosen.

Since closed reference clustering throws out query sequences that cannot be
assigned to reference OTUs, an additional measure of clustering performance to
consider is the fraction of query sequences that were able to be assigned.
...
This mirrors the result reported above that Greengenes produced better OTUs in
...
both MCC score and fraction of query sequences fit.
...
Closed reference OptiFit outperformed OptiClust by 
`r sec_closed_fit_db_vs_clust`% and VSEARCH by `r sec_closed_fit_db_vs_vsearch`%.
In open reference mode, OptiFit ran faster than VSEARCH by 
`r sec_vsearch_vs_open_fit_db`%, but slower than OptiClust by 
`r sec_opticlust_vs_open_fit_db`%.
...
In contrast, OptiFit accepts reference OTUs which each contain multiple
...
**TODO:** The stark difference in OTU definitions between mothur and VSEARCH resulted in...
...
OTUs may be of poor quality when the reference database is too unrelated to the 
samples of interest, such as when samples contain new and rapidly evolving microbes.
For this reason, we were interested in whether we could use the samples themselves
as the reference to cluster against.
