...
database, Silva non-redundant database, or the Ribosomal
Database Project (RDP). **TODO: cite database papers?** In open reference
...
reference alignment [**CITE**], discarding sequences that aligned outside the V4
region, removing chimeric reads with UCHIME, and calculating distances between
all pairs of sequences within each dataset prior to clustering.
To generate reference OTUs from independent databases, we downloaded sequences
from the Greengenes database (v13_8_99) [**CITE**], Silva non-redundant database
(v132) [**CITE**], and the Ribosomal Database Project (RDP; v16) [**CITE**].
...
the reference OTUs generated form each of the three datasets.
When reference clustering with vsearch, processed datasets were fit directly
to the unprocessed greengenes reference alignment, since this is how vsearch
is typically used by the QIIME2 software [@bolyen_reproducible_2019].
...
environment manager [@noauthor_anaconda_2016].
