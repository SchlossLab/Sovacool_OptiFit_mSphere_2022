...
Then for each sequence, OptiClust considers whether the sequence should move to a
different OTU or remain in its current OTU, choosing the option that results in
a better Matthews correlation coefficient (MCC).
...
variation of `r cv_fit_split_mcc_human_simple` for the MCC score across all fractions.
...
but does affect the run time and the fraction of sequences that map during closed reference mode.

**TODO: pick a fraction (e.g. 50%). this part is less important. figure would be supplemental.**
After testing the split strategy using a simple random sample to select the reference sequences, we then investigated other methods of splitting the data.
We tested three methods for selecting the fraction of sequences to be used
...
`r frac_fit_split_1`% of query sequences fit when using 10% of the dataset as 
the reference, to `r frac_fit_split_9`% of query sequences fit when using 90% 
of the dataset as the reference. **TODO: report up to 90% fraction**
