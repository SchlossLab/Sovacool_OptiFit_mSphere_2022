...
- Go back through https://peerj.com/articles/1487/ to get all the arguments for why the qiime based methods are problematic…
...
using the split dataset strategy, while the OTU quality and speed depends on the
database chosen when using the external database strategy.
OptiFit provides a suitable option for users who require stable reference-based
OTU assignments at the same quality afforded by _de novo_ clustering methods.
...
reference-based.
OptiClust, the _de novo_ clustering method provided by the mothur software
program, uses the distance score between all pairs of sequences in the dataset
to cluster them into OTUs by maximizing the Matthews correlation coefficient
(MCC) [@westcott_opticlust_2017].
This approach takes into account the distances between all pairs of sequences
when assigning query sequences to OTUs, in contrast to other tools such as
VSEARCH which only considers the distance between the query sequence and a
representative centroid sequence in the OTU.
...
Furthermore, since _de novo_ clustering requires distance calculations between all
sequences in a dataset, the execution time can be slow for very large datasets.
...
reference sequences are either thrown out (closed reference mode) or clustered
_de novo_ to create additional OTUs (open reference mode).
...
The _de novo_ MCC scores were `r dn_dbs_mcc` for `r names(dn_dbs_mcc)`, 
respectively.
...
(see Figure \@ref(fig:results-sum)).
...
```{r workflow, dpi=300, fig.dim=c(4.5,3.5), fig.cap="**The Analysis Workflow.**"}
# TODO: write caption
here('figures', 'workflow.tiff') %>% 
  tiff::readTIFF() %>% 
  grid::grid.raster() 
```
...
```{r results-sum, dpi=100, fig.dim=c(6,6), fig.cap="**Benchmarking results**"}
# TODO: write caption
here('figures', 'results-sum.tiff') %>% 
  tiff::readTIFF() %>% 
  grid::grid.raster() 
```
...
Thus, the more stringent OTU definition employed by OptiFit resulted in fewer
sequences being fit to reference OTUs than when using VSEARCH, but caused
OptiFit to outperform VSEARCH in terms of both OTU quality and execution time.
...
While we initially tested this strategy using an even split of the data into
reference and query fractions, we then wanted to investigate whether there was
...
These trends hold for the other datasets as well (see supplemental figure **TODO**).
Thus, the reference fraction does not affect OTU quality in terms of MCC score,
but does affect the run time and the fraction of sequences that map during
...
sequences compared to over 200,000 sequences in SILVA and Greengenes each.
...
Unlike existing reference-based methods which map query sequences to a single
...
that does not sacrifice OTU quality for stability of OTU assignments.
