...
with respective average run times of `r closed_fit_db_sec`, `r opticlust_sec`,
and `r open_fit_db_sec` seconds.
...
- we wanted to use x% of data as ref, weren't sure what size to use and strategy for selecting.
...
OTUs quality may be poor when the reference database is too unrelated to the
samples of interest, such as when samples contain low abundant or rapidly
evolving microbes.
While _de novo_ clustering overcomes the quality limitations of reference
clustering to databases, OTU assignments are not stable when new sequences are
added.
...
**TODO; ref weight simple only here. First we wanted to know; does this strategy perform as well as de novo with 50% split?**
...
faster in open reference mode.
**TODO: fraction mapped.**

**Then we wanted to know; what fraction of sequences should be in the reference?**
To test the best reference size, reference sizes from 10% to 80% of the sequences were created, with the remaining sequences used for the query.
