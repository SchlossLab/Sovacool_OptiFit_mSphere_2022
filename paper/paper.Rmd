...
it is limited by the diversity represented in the reference database. _De novo_
methods cluster sequences based on their distance to each other, without the use
of an external reference. open reference methods first perform closed reference
clustering, then any sequences that cannot be assigned to reference OTUs are
clustered _de novo_ to create additional OTUs.  _De novo_ clustering overcomes
...
assignments when new sequences are introduced. Previous studies found that _de
novo_ clustering created the highest quality OTU assignments based on the
Matthews correlation coefficient (MCC) [@schloss_application_2016;
@westcott_novo_2015].
