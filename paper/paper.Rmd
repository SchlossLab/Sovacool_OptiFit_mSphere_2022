...
However, OptiClust and VSEARCH each ran `r sec_opticlust_vs_open_fit_db`% and 
`r sec_vsearch_vs_open_fit_db`% faster than OptiFit in open reference mode.
_De novo_ clustering with OptiClust produced `r mcc_opticlust_vs_vsearch`%
higher quality OTUs than VSEARCH, but performed only 
`r sec_opticlust_vs_vsearch`% as fast as VSEARCH.
