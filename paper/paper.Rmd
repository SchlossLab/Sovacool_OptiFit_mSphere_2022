---
title: "OptiFit: a new method for fitting amplicon sequences to existing OTUs"
date: '`r Sys.Date()`'
...
<!-- TODO
- "require" and "enforce" for opti -- wrong word choice.
- be consistent with cluster, assign, fit, and map.
- can we replace "robust" with something better / more specific?
-->
...
is that OTU assignments from _de novo_ methods may change when new
...
are features.
**TODO**: sentence mentioning open/closed reference clustering and the inherent problems with the current methods.
...
generate millions of reads from microorganisms in diverse natural communities.
...
@rognes_vsearch_2016].
In methods employing greedy clustering algorithms, some pairs of sequences in
the same OTU may have a greater distance than the specified threshold since only
the distance between each sequence and the centroid sequence is considered while
clustering.
In contrast, the OptiClust algorithm enforces that all pairs of sequences must
be within the distance threshold. <!-- i.e. radius vs. diameter -->
...
(e.g. Greengenes) or _de novo_ OTUs and compared the performance to existing
tools.
...
threshold) are counted as true positives if they are assigned to the same OTU,
...
not assigned to the same OTU, and false positives if they are not in the same
OTU.
...
While _de novo_ clustering produces the highest quality OTUs, researchers may
prefer to perform reference clustering to a public database because
reference-based methods produce consistent OTUs and are generally faster than
_de novo_ methods.
...
are added.
Researchers may wish to fit new sequences to existing OTUs or to compare OTUs
across studies.
...
centroid sequence that is found within the distance threshold. TODO: is this true of VSEARCH or only USEARCH?
Because of this, VSEARCH sacrifices OTU quality by allowing more sequences to
fit to OTUs.
...
SOP and accompanying study by Kozich _et al._
...
For each dataset split, the remaining query sequences were assigned to the
