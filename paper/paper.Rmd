...
The result is a robust implementation of closed and open-refernce clustering.
...
reference-based.
OptiClust is a _de novo_ clustering method which uses the distance score between
all pairs of sequences in the dataset to cluster them into OTUs by maximizing
the Matthews Correlation Coefficient (MCC) [@westcott_opticlust_2017].
...
algorithms such as VSEARCH which only considers the distance between the query
sequence and a representative centroid sequence in the OTU [**TODO cite vsearch and schloss papers, also maybe qiime**].
...
[@desantis_greengenes_2006; **TODO cite vsearch**].
...
**TODO: closed uses radius of 3%. de novo uses diameter of 3%. open uses both. vsearch is order dependent.**
...
similarly, with median MCC scores of `r closed_fit_gg_mcc` (**TODO: should be 0.80**) and 
...
execution speed is required, such as when using very large datasets [**TODO: REF**].
...
To test the best reference size, reference sets with 10% to 80% of the
...
This drop in quality was most notable with RDP, which contains only 21,000 (**TODO insert exact number**)
sequences compared to over 200,000 (**TODO insert exact number**) sequences in SILVA and Greengenes each.
...
fit to OTUs.
**TODO: another problem with vsearch is dependence on order of the ref sequences.**
...
[@henson_artificial_2016], mouse gut [@schloss_stabilization_2012], and human
gut [@baxter_microbiota-based_2016] samples.
These were selected because **TODO**.
