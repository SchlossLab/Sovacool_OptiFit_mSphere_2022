...
sequence in an OTU, thus resulting in some dissimilar sequences being clustered
to the same OTU.
...
As a result, pairs of sequences in the same OTU may have a greater distance than
the specified threshold, i.e. they are false positives.
...
Just like in _de novo_ clustering, reference-based methods that use greedy
clustering algorithms only consider the distance between each query and a single
reference sequence, causing some sequences in the same OTU to be more dissimilar
to each other than the specified threshold.
...
_de novo_ methods.
In closed reference mode, sequences that cannot be clustered into reference OTUs
are thrown out, so that the final clustering contains only OTUs that exist in
the reference.
To test how OptiFit performs for this purpose, we clustered each dataset to three
...
both MCC score and fraction of query sequences that could be clustered into the
reference OTUs.
