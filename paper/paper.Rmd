...
**TODO: make sure this info is baked into relevant sections, then remove.**
...
**TODO: ref clustering method paragraph here.**
We assigned blah ref dbs to 3% otus with opticlust. _de novo_ clustering the Greengenes
...
dataset to reference OTUs generated by...
repeated 100 times.
...
MCCs `r mcc_fit_split_simple` and `r mcc_fit_split_abun` respectively), but worse for similarity-weighted
sampling with a median MCC of `r mcc_fit_split_dist`.
In closed reference mode, the fraction of query sequences that can be fit to the
reference OTUs decreases as the reference fraction increases from 
`r frac_fit_split_0.1`% of query sequences fit with `10`% of the dataset as the
reference, to `r frac_fit_split_0.8`% of query sequences fit with `80`% of the
