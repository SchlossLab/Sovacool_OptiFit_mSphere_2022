...
rule silva_fit:
    ...
    params:
        mothur=mothur_bin,
        output_dir="results/{reference}-as-reference/{dataset}/{dataset}_i-{iter}/r-{rep}/method-{method}_printref-{printref}/",
        rep="{rep}",
        method="{method}",
        printref='{printref}'
    benchmark:
        "benchmarks/{reference}-as-reference/{dataset}/{dataset}_i-{iter}/r-{rep}/method-{method}_printref-{printref}/fit.log"
    log:
        "logfiles/{reference}-as-reference/{dataset}/{dataset}_i-{iter}/r-{rep}/method-{method}_printref-{printref}/fit.log"
    shell:
...
rule silva_aggregate_sensspec:
    input:
        ...
        optifit=expand('results/{reference}-as-reference/{{dataset}}/{{dataset}}_i-{iter}/r-{rep}/method-{method}_printref-{printref}/sample.optifit_mcc.sensspec', weight=weights, reference_fraction=reference_fractions, iter=iters, rep=reps, method=methods, printref=printrefs)
    output:
