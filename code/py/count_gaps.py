...
def main():
    ...
    with open(snakemake.output.txt, "w") as outfile:
        outfile.write(
            "n_gaps\ttotal_chars\tdataset\tref_weight\tref_frac\tsample_frac\tseed\n"
        )
        outfile.write(
            f"{n_gaps}\t{total_chars}\t{snakemake.wildcards.dataset}\t{snakemake.wildcards.ref_weight}\t{snakemake.wildcards.ref_frac}\t{snakemake.wildcards.sample_frac}\t{snakemake.wildcards.seed}\n"
        )
