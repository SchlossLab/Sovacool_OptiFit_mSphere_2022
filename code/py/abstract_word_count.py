...
def check_wc(section_name, num_words, word_limit):
    if num_words > word_limit:
        raise ValueError(f"The {section_name} section is {num_words} words. You need to cut {num_words - word_limit} words.")
...
def main(src_filename, log_filename):
    ...
    with open(log_filename, 'w') as outfile:
        ...
        for section, word_limit, starter, stopper in zip(['abstract', 'importance'], 
                                                        [250, 150],
                                    ['## Abstract\n', '### Importance\n'],  ['### Importance\n', "\\newpage\n"]):
