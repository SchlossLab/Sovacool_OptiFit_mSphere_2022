...
def main():  # TODO: write to log file
    ...
    if "sample_frac" in wildcards.__dict__.keys():
        ...
        data_str = (
        f"{wildcards.dataset}\t{wildcards.seed}\t{wildcards.method}\t{wildcards.printref}\t{fraction_mapped}\t{wildcards.fitpercent}\t{wildcards.ref_weight}\n"
                )
        header_line = (
        ...
        header_line = (
        "dataset\tref\tregion\tseed\tmethod\tprintref\tfraction_mapped\n"
        )
