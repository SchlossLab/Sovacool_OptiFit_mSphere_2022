...
get_otu_mat_from_list <- function(otu_list, seqs) {
   otu_mat <- matrix(nrow = length(seqs), ncol = length(seqs), 
...
get_conf_mat <- function(pairs, dist_mat, otu_mat) {
   ...
   conf_mat <- caret::confusionMatrix(table(conditions['pred', ],
                                            conditions['actual', ])) %>%
      as.matrix()
   return(conf_mat)
}
...
dat <- data.frame(Seq1 = c("D", "F", "G", "H", "I", "I", "J", "J", "N", "O", "P", "P", "P", "Q", "Q"), 
...
dist_mat <- matrix(nrow = length(seqs), ncol = length(seqs), 
                   dimnames = list(seqs, seqs), data = 0)
diag(dist_mat) <- 1
for (i in 1:nrow(dat)) {
   r <- dat[i,]
