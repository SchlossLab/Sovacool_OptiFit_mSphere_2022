...
```{python optifit}
import code.py.algorithm_diagram
optifit = create_optifit()
iters = optifit.iterate
dists = dist_sets_to_array(optifit.fitmap.dist_mat)
```
```{r pytor}
...
```{r optifit_example, fig.dim=c(6,5), dpi=300}
...
```{r plot_optifit_iters, fig.dim=c(6.875,6.875), dpi=300}
i <- 0
